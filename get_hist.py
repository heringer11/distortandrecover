...
def tiny_hist_old(image_np):
	...
	image_np_lab_tiny = np.array(Image.fromarray((image_np_lab * 255).astype(np.uint8)).resize((16, 16), Image.BICUBIC)) / 255.0 * 100
...
def example(path):
	...
	img_example = imageio.imread(path).astype('float') / 255.0 - 0.5
