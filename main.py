...
np.set_printoptions(formatter={'float': lambda x: "{0:0.3f}".format(x)}, linewidth=200, threshold=sys.maxsize)
...
class Agent:
	def __init__(self, prefix):
		...
		### MIT5K C
		self.data_dir	= r'D:\MITK5'
		self.test_count = 5 #63
	...
	def init_img_list(self, img_list_path=None, test=False):
		if img_list_path:
			...
			self.train_img_list	= glob.glob(os.path.join(self.train_dir, "raw/*.png"))
			self.test_img_list	= glob.glob(os.path.join(self.test_dir, "raw/*.png"))
	...
	def train_with_queue(self):
		...
		for self.step in tqdm(range(0, self.max_step), ncols=70, initial=0):
			try:
				...
				if self.step == 0:
					# dry test
					print("Performing dry test")
					self.test(idx=0)

				self.q_learning_minibatch()
				# print('Completed learning minibatch')
	...
	# 	##################
	# 	self.s_t_single			= tf.compat.v1.placeholder('float32', [self.batch_size, self.feature_length], name="s_t_single")
	# 	self.s_t_plus_1_single	= tf.compat.v1.placeholder('float32', [self.batch_size, self.feature_length], name="s_t_plus_1_single")
	# 	self.action_single		= tf.compat.v1.placeholder('int64', 	[self.batch_size, 1], name='action_single')
	# 	self.terminal_single	= tf.compat.v1.placeholder('int64', 	[self.batch_size, 1], name='terminal_single')
	# 	self.reward_single		= tf.compat.v1.placeholder('float32', [self.batch_size, 1], name='reward_single')


	# 	self.s_t		= tf.compat.v1.placeholder('float32', [self.batch_size, self.feature_length], name="s_t")
	# 	self.target_q_t	= tf.compat.v1.placeholder('float32', [self.batch_size], name='target_q_t')
	# 	self.action		= tf.compat.v1.placeholder('int64', [self.batch_size], name='action')
	...
	# 	# self.queue = tf.queue.FIFOQueue(self.memory_size, [tf.float32, tf.float32, tf.float32, tf.int64, tf.int64])
	# 	self.queue = tf.queue.RandomShuffleQueue(self.memory_size, 1000, [tf.float32, tf.float32, tf.float32, tf.int64, tf.int64], shapes=[[self.feature_length], [self.feature_length], [1], [1], [1]])
	...
	# 	self.target_s_t = tf.compat.v1.placeholder('float32', [self.batch_size, self.feature_length], name="target_s_t")

	# 	if self.use_batch_norm:
	# 		self.phase_train = tf.compat.v1.placeholder(tf.boolean,name='phase_train')
	...
	# 	self.q_action = tf.argmax(self.q, axis=1) # is dimension really 1??
	...
	# 		for weight in list(self.q_w.values()):
	# 			self.loss += 1e-4*tf.nn.l2_loss(weight)
	# 		# self.learning_rate_op = tf.maximum(self.learning_rate_minimum,
	# 		# 		tf.keras.optimizers.schedules.ExponentialDecay( 	self.learning_rate, self.global_step, 
	...
	# 		staircase=True
	# 		)
	# 		self.learning_rate_op = tf.maximum(self.learning_rate_minimum, learning_rate_schedule(self.global_step))
	...
	# 		# optimizer = tf.keras.optimizers.Adam(learning_rate=self.learning_rate_op)
	# 		# self.optim = optimizer.minimize(self.loss, var_list=list(self.q_w))
	# 		# learning_rate_value = sess.run(self.learning_rate_op)  # Avalia o tensor para obter o valor da taxa de aprendizagem
	# 		# optimizer = tf.keras.optimizers.Adam(learning_rate=learning_rate_value)
	# 		# self.optim = optimizer.minimize(self.loss, var_list=list(self.q_w))
	# 		self.optim = tf.compat.v1.train.AdamOptimizer(self.learning_rate_op).minimize(self.loss, var_list=list(self.q_w.values()), global_step=self.global_step)
	...
	# 			self.target_q_w_input[name] = tf.compat.v1.placeholder('float32', self.target_q_w[name].get_shape().as_list(), name=name)
	...
	# 	self.saver = tf.compat.v1.train.Saver(list(self.q_w.values()) + [self.step_op], max_to_keep=30)
	# 	#if model_path:
	# 	# restore the model
	...
	def init_model(self, model_path=None):
		...
		if self.use_batch_norm:
			self.phase_train = tf.compat.v1.placeholder(tf.boolean, name='phase_train')
			self.q, self.q_w = q_network(self.s_t, 'pred', input_length=self.feature_length, num_action=self.action_size, duel=self.duel, batch_norm=self.use_batch_norm, phase_train=self.phase_train)
			self.target_q, self.target_q_w = q_network(self.target_s_t, 'target', input_length=self.feature_length, num_action=self.action_size, duel=self.duel, batch_norm=self.use_batch_norm, phase_train=self.phase_train)
		else:
			self.q, self.q_w = q_network(self.s_t, 'pred', input_length=self.feature_length, num_action=self.action_size, duel=self.duel)
			self.target_q, self.target_q_w = q_network(self.target_s_t, 'target', input_length=self.feature_length, num_action=self.action_size, duel=self.duel)
		...
		tf.compat.v1.global_variables_initializer().run()
		self.saver = tf.compat.v1.train.Saver(list(self.q_w.values()) + [self.global_step], max_to_keep=30)
	...
	def update_target_q_network(self):
		with tf.compat.v1.Session() as sess:
			...
			for name in self.q_w.keys():
				...
				try:
					# self.target_q_w_assign_op[name].eval(feed_dict={self.target_q_w_input[name]: self.q_w[name].eval()})
					sess.run(self.target_q_w_assign_op[name], feed_dict={self.target_q_w_input[name]: self.q_w[name].eval()})
				except Exception as e:
	...
	def _load_images(self, offset, is_training,in_order=False,get_raw_images=False):
		...
		for img_path in img_list:
			imgs.append(np.array(Image.fromarray(imageio.imread(img_path)).resize((224, 224)))/ 255.0 - 0.5)
			...
			target_path = target_path.split('.')[0] + '.jpg'
			if "__" in os.path.basename(img_path):
				op_str.append(os.path.basename(img_path).split("__")[1])
			else:
				op_str.append("_")
			target_imgs.append(np.array(Image.fromarray(imageio.imread(target_path)).resize((224, 224)))/ 255.0 - 0.5)

			if get_raw_images:
				raw_imgs_raw.append(np.array(Image.fromarray(imageio.imread(img_path)))/ 255.0 - 0.5)
				raw_imgs_target.append(np.array(Image.fromarray(imageio.imread(target_path)))/ 255.0 - 0.5)
	...
	def test(self, is_batch_test=False, use_target_q=False, in_order=False, idx=-1):
		...
		if is_batch_test:
			...
			test_result_dir =  "test/"+self.prefix+"/step_" + str(self.step)
			#r"test\test_run\step_" + str(self.step)
		...
		for i in range(state_raw.shape[0]):
			...
			try:
				...
				with open(os.path.join(test_result_dir, '%s.log'%fn[i]), 'wb') as f:
					log_entry = "%s_%f_%s.png\n" % (fn[i], score_diff[i][0], actions_desc)
					f.write(log_entry.encode('utf-8'))  # Converte a string para bytes usando UTF-8
					print('write test results dir')
...
if __name__ == '__main__':
	...
	try:
		...
		with tf.compat.v1.Session(config=config) as sess:
			...
			print('init_preprocessor')
			agent.init_model()
			print("init model")
			agent.init_img_list()
			print('init image list')
			if model_path:
